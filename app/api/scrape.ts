...
export const scrape = async(cookie : string, keyword : string, videos_count : number = 100) => {
    try {
        const youtube = new Client({initialCookie: `${cookie}`});
        let result : SearchResult < "video" > = await youtube.search(`${keyword}`, {type: "video"});
        let allItems : VideoItem[] = result
            .items
            .map((item : VideoCompact) => ({
                title: item.title,
