...
export async function POST(request : NextRequest) {
    ...
    if (scraped_result
        ?.items && scraped_result.items.length > 0) {
        scraped_result
            .items
            .forEach(async(item) => {
                // Check if the item already exists in the database
                const existingItem = await ItemModel.findOne({video_id: item.video_id, keywordGroup: keywordobject.superset._id});
        ...
        return NextResponse.json({
            message: `Scrapped : ${scraped_result
                ?.length} videos!`
        });
    }
    // Create a new user console.log(existingUser, keywordobject.superset.keyword)
