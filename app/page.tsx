...
export default function Page() {
  ...
  const [keywordSuperset, setkeywordSuperset] = useState("");
  const [keywordSupersetid, setkeywordSupersetid] = useState("");
  const [dropDown, setdropDown] = useState("Select");
  const [keywords, setKeywords] = useState<keywordtype[]>([]);
  const [keywordGroups, setKeywordGroups] = useState<keywordtype[]>([]);
  ...
  const [selectedKeywordGroupToAssign, setselectedKeywordGroupToAssign] =
    useState("");
  ...
  const fetchCookie = async () => {
    const response = await fetch(
      `/api/admin/users/cookie?userId=${session?.user.id}`
    );
    if (response.ok) {
  ...
  const addKeyword = async () => {
    const response = await api.post("/api/admin/keywords/add", {
      keyword,
      keywordGroupId: keywordSupersetid,
    });
    if (response.status === 200) {
      setKeywords((prevKeywords: any) => [...prevKeywords, keyword]);
  ...
  const addGroup = async () => {
    const response = await api.post("/api/admin/keywords/group/add", {
      keywordSuperset,
    });
    if (response.status === 200) {
      setKeywordGroups((prevKeywords: any) => [...prevKeywords, keyword]);
  ...
  const removeKeyword = async () => {
    const response = await api.delete(
      `/api/admin/keywords/remove?keyword=${dropDown}`
    );
    if (response.status === 200) {
      setKeywords((prevKeywords: any) =>
        (prevKeywords || []).filter((keyword: any) => keyword.name !== dropDown)
      );
  ...
  const showItemsbyGroup = async (selectedKeywordGroup: any) => {
    ...
    const response = await api.get(
      `/api/admin/keywords?keywordGroupId=${selectedKeywordGroup}`,
      {
        timeout: 500000,
      }
    );
    ...
    <div className="flex flex-col items-center justify-center  bg-gray-800 text-white">
      <div className="flex flex-row justify-center items-center space-x-4">
        <div className="flex flex-col items-center justify-center mt-14">
          ...
          <button
            onClick={sendCookie}
            className="p-2 bg-blue-500 text-white rounded hover:bg-blue-600 mt-1"
          >
            Set Cookie
          </button>
        </div>
      ...
      <div>
            ...
            <div className="w-full">
              <CustomGroupSearch
                ...
                onChange={(selectedOption) =>
                  handleKeywordGroupChange(selectedOption)
                }
              />
            </div>
          </div>
        ...
        {!isLoading && (
          <div className="flex flex-row items-center justify-center space-x-4">
            <button
              ...
              className="mt-4 p-2 bg-blue-500 text-white rounded hover:bg-orange-600"
            >
              Scrape Keyword
            </button>
            <button
        ...
        {isLoading && (
          <div>
            {" "}
            <span className="loading loading-bars loading-lg"></span>Scrapping
            ....{" "}
          </div>
        )}
      </div>
      ...
      <div className="flex">
        <div className="mt-7 min-w-fit">
          <CustomGroupSearch
            options={keywordGroups}
            onChange={(selectedOption) =>
              handleShowScrappedBygroup(selectedOption)
            }
          />
        </div>
        {items?.length > 0 && (
          <div className="text-center mt-8 text-red-900">
      ...
      >
        {items?.length === 0 && (
          <div className="text-center mt-8 text-red-900">No items found</div>
        )}
        <table className="mt-8 w-4/5 text-center text-red-900 border-2 border-blue-300 border-collapse mx-auto bg-white">
          <thead>
            <tr>
